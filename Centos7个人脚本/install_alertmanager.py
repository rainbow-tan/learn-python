...
def get_install_package(start_with):
    ...
    for file in all_files:
        if os.path.basename(file).startswith(start_with) and os.path.basename(file).endswith(".tar.gz"):
            ...
            return file
    msg = f"未找到'{start_with}'安装包, 去官网'https://prometheus.io/download/#alertmanager'下载后, 放在当前目录!!!"
    raise Exception(msg)
...
if __name__ == '__main__':
    main()
    """
    Centos7 安装 alertmanager
    使用方法:
    以alertmanager-0.25.0.linux-amd64.tar.gz为例
    把alertmanager-0.25.0.linux-amd64.tar.gz放在当前目录, 没有就去'https://prometheus.io/download/#alertmanager'下载
    先保证远程机器的SSH可用,然后配置文件中的四个字段
    ip = '172.16.70.42'
    port = 22
    username = 'root'
    password = '111111'
    运行python3 install_alertmanager.py
    """